...
function generatePassword() {
  ...
  if (length < 6 || length > 32) {
    errorMsg.textContent =
      "La longitud de la contraseña debe estar entre 6 y 32 caracteres.";
    return;
  } else if (
    !includeLowercase &&
    !includeUppercase &&
    ...
    errorMsg.textContent =
      "Debe seleccionar al menos una opción de caracteres.";
    return;
  } else {
    errorMsg.textContent = "";
  }
...
// Copiar contrasena a clipboard
copyButton.addEventListener("click", function () {
  const passwordInput = document.getElementById("password");
  passwordInput.select();
  document.execCommand("copy");
  alert("Contraseña copiada al portapapeles amigo");
});
